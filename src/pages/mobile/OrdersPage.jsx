...
import { NavBar, Tabs, Empty, Toast } from 'antd-mobile'
...
function OrdersPage() {
  ...
  const fetchOrders = useCallback(async (status) => {
    ...
    setLoading(true)
    try {
      const data = await getOrders({ status: status === 'all' ? undefined : status })
  ...
  const handleCancel = async (orderId) => {
    try {
      ...
      Toast.show({ icon: 'fail', content: err.response?.data?.error || '取消失败' })
    }
  }
        ...
        <>
          ...
          <div className="orders-list">
            {loading ? (
              ...
              orders.map(order => (
                <div key={order.id} className="order-card" onClick={() => navigate(`/pay/${order.id}`)}>
